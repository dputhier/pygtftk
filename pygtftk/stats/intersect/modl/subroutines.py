...
def learn_dictionary_and_encode(data, n_atoms = 20, alpha = 0.5,
        n_iter = 100, random_seed = 42, n_jobs = 1):
    """
    ...
    if np.isnan(V).any():
        message("Learned dictionary contained NaNs. Alpha was likeky inadapted. Aborting.")
...
def build_best_dict_from_library(data, library, queried_words_nb,
    ...
    while stop is False:
        ...
        for candidate in all_candidate_words:
            ...
            with warnings.catch_warnings():
                # Ignore convergence warnings likely due to zero alpha
                warnings.filterwarnings("ignore", category=ConvergenceWarning)
        ...
        # TO save time, we may only pass as candidates the children (see tree)
